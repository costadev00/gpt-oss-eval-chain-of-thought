...
\title{Avaliação do GPT-OSS-20B com Chain-of-Thought e Process Supervision Promptado}
\author{Matheus Costa Monteiro}
\date{28 de março de 2026}

\begin{document}
\sloppy
\makedendentitle{PCS5029 - PLN com RNN}{Relatório: GPT-OSS-20B com Chain-of-Thought e Process Supervision}{}
...
com majority vote e nao supera o baseline simples. Todo o codigo usado para
gerar, avaliar e documentar os experimentos esta disponivel no repositorio
publico do projeto.
\end{abstract}
...
Artefatos CoT & \texttt{experiments/01\_chain\_of\_thought/} \\
Artefatos PRM & \texttt{experiments/02\_process\_supervision/} \\
\bottomrule
\end{tabularx}
\end{table}
...
\section{Codigo, organizacao e artefatos}

Todo o codigo-fonte, a documentacao de reproducao e os artefatos resumidos
deste trabalho estao disponiveis no repositorio GitHub~\cite{repo}:
\href{https://github.com/costadev00/gpt-oss-eval-chain-of-thought}{repositorio
publico do projeto}. O
repositorio foi organizado para separar claramente os dois experimentos:
\texttt{01\_chain\_of\_thought} contem a avaliacao de CoT, e
\texttt{02\_process\_supervision} contem o teste de Process
Supervision promptado. O arquivo \texttt{README.md} funciona como guia de
execucao, enquanto este artigo consolida a metodologia, os resultados e a
analise critica.

\begin{table}[h]
\centering
\caption{Principais arquivos do repositorio usados neste estudo.}
\label{tab:repo_files}
\begin{tabularx}{\textwidth}{p{0.27\textwidth}X}
\toprule
Arquivo ou pasta & Papel no projeto \\
\midrule
\texttt{README.md} & Guia principal em portugues, com instalacao, comandos,
metodologia resumida, hardware e resultados. \\
\texttt{pyproject.toml} & Metadados do pacote Python, dependencias e
configuracao de testes. \\
\texttt{run.py} & Ponto de entrada do experimento CoT versus resposta
direta. \\
\texttt{prompts.py} & Prompts e contrato de saida usados nas
condicoes \emph{standard} e CoT. \\
\texttt{tasks.py} & Carregamento de GSM8K e geracao deterministica
das tarefas Last Letter e Coin Flip. \\
\texttt{audit\_parser.py} & Auditoria offline do parser sem nova
chamada ao modelo. \\
\texttt{prm\_run.py} & Ponto de entrada do experimento de Process
Supervision promptado. \\
\texttt{math\_tasks.py} & Carregamento e normalizacao numerica do
benchmark MATH. \\
\texttt{prm\_scoring.py} & Divisao de solucoes em passos, calculo de
scores PRM e majority vote. \\
\texttt{01\_chain\_of\_thought} & Pasta em \texttt{experiments/} com README
proprio e artefatos preservados da rodada principal de CoT. \\
\texttt{02\_process\_supervision} & Pasta em \texttt{experiments/} com README
proprio e artefatos preservados da rodada principal de Process Supervision. \\
\texttt{relatorio .tex} & Fonte deste artigo em \texttt{reports/}. \\
\bottomrule
\end{tabularx}
\end{table}

As saidas brutas completas, como \texttt{predictions.jsonl},
\texttt{candidates.jsonl}, \texttt{selected.jsonl} e
\texttt{step\_scores.jsonl}, nao sao versionadas no repositorio enxuto porque
podem ser regeneradas pelos comandos documentados. Para manter rastreabilidade
sem inflar o repositorio, preservamos os arquivos pequenos de configuracao,
metricas e analise nas pastas de cada experimento.
...
A implementacao deste experimento esta concentrada em \texttt{cot\_eval/run.py}.
Os prompts ficam em \texttt{cot\_eval/prompts.py}, as tarefas em
\texttt{cot\_eval/tasks.py} e as funcoes de extracao/comparacao de respostas em
\texttt{cot\_eval/scoring.py}. Os artefatos preservados da rodada principal
ficam em \texttt{artifacts/20260510-185218/}, dentro da pasta
\texttt{experiments/01\_chain\_of\_thought/}.
...
A implementacao deste estudo esta em \texttt{cot\_eval/prm\_run.py}. O
carregamento e a normalizacao do MATH ficam em \texttt{cot\_eval/math\_tasks.py};
os prompts de geracao e julgamento ficam em \texttt{cot\_eval/prm\_prompts.py};
e a divisao em passos, o calculo dos scores e o majority vote ficam em
\texttt{cot\_eval/prm\_scoring.py}. Os artefatos preservados da rodada
principal ficam em \texttt{artifacts/20260510-231913/}, dentro da pasta
\texttt{experiments/02\_process\_supervision/}.
...
Todos os scripts usados para executar essas avaliacoes, os testes automatizados,
os artefatos resumidos e o README de reproducao estao disponiveis no repositorio
publico do projeto~\cite{repo}:
\href{https://github.com/costadev00/gpt-oss-eval-chain-of-thought}{GitHub do
projeto}.
...
\newblock \href{https://huggingface.co/datasets/openai/gsm8k}{Hugging Face:
openai/gsm8k}.

\bibitem{math}
EleutherAI.
\newblock Hendrycks MATH dataset.
\newblock \href{https://huggingface.co/datasets/EleutherAI/hendrycks_math}{Hugging
Face: MATH}.

\bibitem{repo}
CostaDev00.
\newblock GPT-OSS eval chain-of-thought repository.
\newblock \href{https://github.com/costadev00/gpt-oss-eval-chain-of-thought}{Repositorio
GitHub do projeto}.
